...
export class OpenVasParser extends BaseScanner {
  ...
  async scan(_context: ScannerContext): Promise<string> {
    ...
    throw new Error('OpenVAS scanning is handled by the GVM API integration. Use the OpenVasService.');
  }

  async parse(rawOutput: string, context: ScannerContext): Promise<NormalizedFinding[]> {
    const findings: NormalizedFinding[] = [];
    if (!rawOutput.trim() || !rawOutput.includes('<report')) return findings;

    const parser = new XMLParser({
      ignoreAttributes: false,
      attributeNamePrefix: '@_',
      isArray: (name) => ['result', 'nvt', 'refs', 'ref', 'solution'].includes(name),
    });
    ...
    for (const r of results) {
      ...
      const severity = cvssScore > 0 ? this.cvssScoreToSeverity(cvssScore) : this.mapSeverity(r.threat ?? 'info');
      ...
      findings.push({
        ...
        port: r.port ? parseInt(r.port.split('/')[0] ?? '0', 10) : undefined,
        protocol: r.port ? r.port.split('/')[1]?.toLowerCase() : undefined,
